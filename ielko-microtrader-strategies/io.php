...
db_query($sql) or die(db_error());
          ...
          case 'get_data':
$sql = "select * from `stats` where apikey = 'ZWZjNzE5ZDY5MzAxNDBmMTk0NGJjYzNj' and strategy LIKE '%DOGE%' and timestamp BETWEEN SUBDATE(CURDATE(), INTERVAL 1 MONTH) AND NOW();";
$sql = preg_replace("/(?<=[A-Za-z0-9])(')(?=[A-Za-z0-9])/", "\'", $sql);
$result = db_query($sql);
